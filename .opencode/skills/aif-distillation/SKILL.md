...
- optional `--path <directory>` to save generated skill package directories under a custom output root instead of `.opencode/skills`
...
- Resolve the output root: `--path <directory>` when present, otherwise `.opencode/skills`.
...
Default destination for single-skill mode: `<output-root>/<skill-name>/`, where `<output-root>` is `.opencode/skills` unless `--path` is present.
...
1. Prepare sources.
   ...
   - For large folders or PDFs, use `.opencode/skills/aif-distillation/scripts/material-prep.py` to extract and chunk material, then clean temporary extraction artifacts with the helper after the skill is generated.
...
4. Design the target skill package.
   ...
   - Resolve `<output-root>` from `--path` or `.opencode/skills` before writing. Treat `--path` as a parent directory for generated skill packages, not as the skill package name.
...
- Primary ownership: generated or updated skill packages under `<output-root>/<skill-name>/`, or multiple direct child skill packages under `<output-root>/<prefix>-<child-scope>/` in split mode. `<output-root>` is `.opencode/skills` unless the user passes `--path <directory>`.
