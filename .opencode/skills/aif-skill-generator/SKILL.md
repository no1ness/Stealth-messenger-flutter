...
0. Scope check (MANDATORY):
   ...
   - If path points to built-in AI Factory skills (.opencode/skills/aif or .opencode/skills/aif-*), this is wrong target selection for install-time security checks.
...
2. LEVEL 1 — Run automated scan:
   $PYTHON ~/.opencode/skills/aif-skill-generator/scripts/security-scan.py <skill-path>
...
2. **LEVEL 1** — Run automated scanner:
   ```bash
   $PYTHON ~/.opencode/skills/aif-skill-generator/scripts/security-scan.py <path>
   ```
...
```bash
$PYTHON ~/.opencode/skills/aif-skill-generator/scripts/security-scan.py ./skill-name/
```
...
```bash
python ~/.opencode/skills/dependency-graph/scripts/visualize.py $ARGUMENTS
```
```
...
1. **Local**: Keep in `~/.opencode/skills/` for personal use
2. **Project**: Add to `.opencode/skills/` and commit
