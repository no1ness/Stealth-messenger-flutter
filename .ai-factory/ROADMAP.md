...
- [x] **M3 — PocketBase identity hardening.** `users.id == pbIdFromLocalUuid(selfUserId)` (15-char SHA-256), symmetric `creatorUuid` in payloads, incoming-call auth race fixed. Transplanted from `fix/pb-user-id-collision-fit` into `main` (commits `3e733f9`, `7b9c0a0`). (`post-pocketbase-hardening` Phase 1.)
...
- [x] **M10 — Design system v2 (Apple Liquid).** (Superseded by M19 — Telegram-tt flat design.) Tokens, primitives, signature effects (ScanlineOverlay, GrainOverlay, ChromaticAberration, DecryptText, key-fingerprint backdrop), `GlassPageRoute`, `StealthHaptics`. Source of truth: `docs/design-system.md` + `docs/design-mockups/`. Backing plans: `.ai-factory/plans/feature-ui-design-refactor.md` (6 refinement passes shipped) + `.ai-factory/plans/feature-russian-ui-design-alignment.md`.
- [x] **M11 — Web rollout readiness + perf benchmarks.** Font subsetting via `pyftsubset` (Geist + Geist Mono → used-glyph subset; 90% size reduction ~165KB→~18KB per font), web-specific perf gates (BackdropFilter audit — clean, no usage), **Performance benchmark harness** (FPS, load time, bundle size, render speed), **monitoring dashboard** (device info, install tracking, platform stats, P2P/WebRTC connection stats, message/call/file counts) deployed at dashboard.stealthpro.ru. Backing plans: `.ai-factory/plans/feature-dashboard-web-monitoring.md` + `.ai-factory/plans/perf-benchmarks-monitoring.md`. **2026-06-21.**
- [x] **M20 — E2E Test optimization & Dashboard.** Оптимизация pw-test (замена устаревшего .type на .fill, expect-ожидания) и создание локального Test Dashboard (Vanilla Node.js / HTML SPA) для удобного управления Playwright тестами. **2026-06-26.**

## In flight

- [~] **M9.1 — Re-enable Android CI build-apk.** Scoped under M9 (Android release hardening). `ci.yml` `if: false` gate removed (originally gated in commit `06dbd4c` for slowness; gradle cache now warm). Branch `chore/ci-reenable-android-build` has diverged significantly from main — needs reconciliation.

- [~] **M19 — Design system v3 (Telegram-tt flat).** Замена Apple Liquid на плоский Telegram-дизайн (telegram-tt). Удалены: `apple_liquid/`, Geist-шрифты, glass-эффекты (Scanline, Grain, ChromaticAberration), GlassPageRoute, StealthAnimatedBackground. Добавлены: `TgThemeColors` (~76 токенов), `TgThemeData` light/dark, `TgSpacing`, `TgTypography` (22 стиля), `FlatContainer`, `TgAppBar`, `TgBottomNavBar`, `TgChatBubble`, `TgChatTile`, `TgContactTile`, `TgSearchField`, `TgMessageInput`, Feedback (TgSnackBar/TgDialog/TgLoading/TgHaptics). Шрифты: Roboto (системный) вместо Geist. 259 тестов, `flutter analyze` 0 errors, `flutter build web` успешен. Backing plan: `.ai-factory/plans/feature-telegram-tt-design.md`.
...
- [ ] **M13 — Appium suite sync.** AccessibilityIds contract preserved through M19 (Telegram-tt). Sync the values with the out-of-repo Appium suite. Trigger: post-M19 merge.
