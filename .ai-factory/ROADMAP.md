...
- [~] **M17 — In-app diagnostics & logs viewer.** Settings → "Open diagnostics & logs" surfaces five live service health rows (database via `secureStorageReady`, attachments via `fileCount`, identity, p2p via new public `activeChannelCount`+`retryWorkerRunning` getters, pocketbase via direct `dotenv` read). In-memory log ring buffer (capacity 500) in `lib/logging/log_buffer.dart` drives a filterable WARN/ERROR view; "Share logs" exports via `share_plus` with mandatory `scrubInlineSensitive` redaction (UUID v4 / 15-char PB id / base64 pubkey patterns with entropy heuristics) so identifiers interpolated into raw log messages can't leak to third-party messengers. Backing plan: `.ai-factory/plans/diagnostics-logs-screen.md` (5 `/aif-improve` refinement passes shipped). Branch `feature/diagnostics-logs-screen` pushed (commit `46fc518`); PR pending. Task 14 (manual smoke on web + Android share intent + redaction verification in a real messenger) remains the only open plan item.

## Committed, not started

- [ ] **M11 — Web rollout readiness.** Font subsetting via `pyftsubset` (Geist + Geist Mono → used-glyph subset; expected 60–70% bundle reduction), web-specific perf gates beyond GlassTextField (BackdropFilter-heavy surfaces audit), manual smoke matrix (Chrome / Safari / Firefox). Trigger: stakeholder decision that web is a release target.
