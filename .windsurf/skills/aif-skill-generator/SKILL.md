...
0. Scope check (MANDATORY):
   ...
   - If path points to built-in AI Factory skills (.windsurf/skills/aif or .windsurf/skills/aif-*), this is wrong target selection for install-time security checks.
...
2. LEVEL 1 — Run automated scan:
   $PYTHON ~/.windsurf/skills/aif-skill-generator/scripts/security-scan.py <skill-path>
...
2. **LEVEL 1** — Run automated scanner:
   ```bash
   $PYTHON ~/.windsurf/skills/aif-skill-generator/scripts/security-scan.py <path>
   ```
...
```bash
$PYTHON ~/.windsurf/skills/aif-skill-generator/scripts/security-scan.py ./skill-name/
```
...
```bash
python ~/.windsurf/skills/dependency-graph/scripts/visualize.py $ARGUMENTS
```
```
...
1. **Local**: Keep in `~/.windsurf/skills/` for personal use
2. **Project**: Add to `.windsurf/skills/` and commit
