...
export default defineConfig(({ mode }): UserConfig => {
  ...
  const defaultAppTitle = `Stealth${appEnv !== 'production' ? ' Beta' : ''}`;
  ...
  const pocketbaseUrl = env.VITE_POCKETBASE_URL || 'http://localhost:8090';
  ...
  setViteEnv({
    ...
    TG_POCKETBASE_URL: pocketbaseUrl,
...
function buildCsp(appEnv: string) {
  return `
  default-src 'self';
  connect-src 'self' blob: http: https: ${appEnv === 'development' ? 'wss: ipc:' : ''};
