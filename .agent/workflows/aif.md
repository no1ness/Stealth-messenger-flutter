...
- Never run blocking security decisions on built-in AI Factory skills (`~/.agent/skills/aif` and `~/.agent/skills/aif-*`).
...
```bash
$PYTHON ~/.agent/skills/aif-skill-generator/scripts/security-scan.py <installed-skill-path>
```
...
For each recommended skill:
  ...
  2. If found → Install: npx skills install --agent antigravity <name>
...
```bash
node ~/.agent/skills/aif/references/update-config.mjs \
  --template ~/.agent/skills/aif/references/config-template.yaml \
...
3. Run `node ~/.agent/skills/aif/references/update-config.mjs --template ~/.agent/skills/aif/references/config-template.yaml --target .ai-factory/config.yaml --payload .ai-factory/config.update.json`
...
7. For each external skill from skills.sh:
   ```bash
   npx skills install --agent antigravity <name>
   # AUTO-SCAN: immediately after install
   $PYTHON ~/.agent/skills/aif-skill-generator/scripts/security-scan.py <installed-path>
   ```
