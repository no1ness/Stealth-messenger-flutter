...
- Never run blocking security decisions on built-in AI Factory skills (`~/.windsurf/skills/aif` and `~/.windsurf/skills/aif-*`).
...
```bash
$PYTHON ~/.windsurf/skills/aif-skill-generator/scripts/security-scan.py <installed-skill-path>
```
...
For each recommended skill:
  ...
  2. If found → Install: npx skills install --agent windsurf <name>
...
```bash
node ~/.windsurf/skills/aif/references/update-config.mjs \
  --template ~/.windsurf/skills/aif/references/config-template.yaml \
...
3. Run `node ~/.windsurf/skills/aif/references/update-config.mjs --template ~/.windsurf/skills/aif/references/config-template.yaml --target .ai-factory/config.yaml --payload .ai-factory/config.update.json`
...
7. For each external skill from skills.sh:
   ```bash
   npx skills install --agent windsurf <name>
   # AUTO-SCAN: immediately after install
   $PYTHON ~/.windsurf/skills/aif-skill-generator/scripts/security-scan.py <installed-path>
   ```
